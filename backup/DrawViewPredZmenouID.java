...
public class DrawView extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		drawGrid(canvas);
		//Collections.sort(appListToDraw, new ApplicationComparatorByZ());
	...
	private void drawPerformanceInfo(Canvas canvas) {
		if (MainActivity.sharedPref.getBoolean("ViewPerformanceInfo", true)) {
			float[][] perfInfo = new float[4][3];
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 3; j++) {
					perfInfo[i][j] = i * j;
				}
			...
			for (int i = 0; i < perfInfo.length; i++) {
				for (int j = 0; j < perfInfo[0].length; j++) {
					canvas.drawText(perfInfo[i][j] + "", sageLeft + i * 50, sageBottom + 20 + j * 30, paint);
				}
			}
		}
	}
